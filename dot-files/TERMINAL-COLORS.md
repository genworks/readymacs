...
WSL2 Ubuntu-24.04 + bash + Readymacs installed on the host.
...
| bash | `LS_COLORS` (ls), `PS1` (prompt), via the `theme` function — all **24-bit truecolor**, so they bypass the WT palette entirely | `~/.bash_profile` (symlink → `readymacs/dot-files/bash_profile`), `~/.bashrc` |
| Emacs | Its own faces (modus-vivendi / modus-operandi); in GUI and truecolor terminals it ignores the WT palette too | `emacs.d` in readymacs |
