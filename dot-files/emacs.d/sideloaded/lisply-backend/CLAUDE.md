...
1. **The baked index** -- `lisply-search-config.sexp` beside this file,
   `:lisply-search-config` → `:sources`: `readymacs` (this console's
...
2. **Engine corpora** -- each project builds its own corpus with the
   reference indexer (`lisply-index`, on the path in every Readymacs
   image) and ships it in its image under the `lisply.corpus` label
   (Gendl: `/opt/gendl/lisply-corpus/gendl.sexp`). The deployment
   copies the files out of the images it runs into the corpora
   directory.
3. **Mount corpora** -- what the deployment builds itself from
   `/projects` (the live demos, the training material, internal
   applications on internal deployments), declared in the deployment
   configuration and written into the same directory at start.
...
corpus there REPLACES a same-named baked source: in a deployment the
Gendl corpus is the Gendl actually running, not the one this image was
...
index in a dev console with `M-x lisply-search-build-index`; a corpus
...
mount corpora on internal deployments.
