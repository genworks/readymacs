...
;; generate-skewed-dashboard-banner.  This fork's default is the
;; Genworks attribution.
(setq dashboard-footer-messages
      (list (or (getenv "SKEWED_DASHBOARD_FOOTER")
                "Brought to you by 𝙶𝚎𝚗𝚠𝚘𝚛𝚔𝚜 𝙸𝚗𝚝𝚎𝚛𝚗𝚊𝚝𝚒𝚘𝚗𝚊𝚕")))
   ...
   (concat "    "
           (propertize (concat (skewed-dashboard-pad-icon :help-target)
                       ...
                       'keymap (let ((map (make-sparse-keymap))
                                 ...
                                 map)
                       'face 'button
                       'help-echo "Open Daily Focus agenda (first use sets up starter org files)"))
